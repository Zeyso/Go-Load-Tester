...
func (mf *MinecraftFlooder) GetServers() []string {
	return mf.servers
}

func (mf *MinecraftFlooder) GetProxies() []utils.ProxyConfig {
	// Konvertiere die internen Proxies zu utils.ProxyConfig
	result := make([]utils.ProxyConfig, len(mf.proxies))
	for i, p := range mf.proxies {
		result[i] = utils.ProxyConfig{
			Protocol: p.Protocol,
			Host:     p.Host,
			Port:     p.Port,
			Username: p.Username,
...
func (mf *MinecraftFlooder) GetConcurrency() int {
	return mf.concurrency
}

func (mf *MinecraftFlooder) GetDuration() time.Duration {
	return mf.duration
}

func (mf *MinecraftFlooder) GetFloodType() string {
	return mf.floodType
}

func (mf *MinecraftFlooder) GetUseRotating() bool {
	return mf.useRotating
}

// GetErrorStats gibt eine Kopie der aktuellen Fehlerstatistiken zurück
func (mf *MinecraftFlooder) GetErrorStats() map[string]int64 {
	mf.errorMutex.RLock()
	defer mf.errorMutex.RUnlock()
	stats := make(map[string]int64)
